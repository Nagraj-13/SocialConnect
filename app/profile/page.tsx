    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <Card className="shadow-md hover:shadow-lg transition">
        <CardContent className="flex flex-col md:flex-row items-center gap-6 p-6">
          ...
          <div className="flex-1 text-center md:text-left space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Joined {new Date(user.created_at).toDateString()}</p>

            {/* Stats */}
            <div className="flex gap-6 mt-2 justify-center md:justify-start text-sm">
              <Button onClick={() => setOpenFollowers(true)} className="hover:underline  bg-transparent text-foreground hover:bg-transparent hover:text-foreground">
                <strong>{user.followers.length}</strong> Followers
              </Button>
              <Button onClick={() => setOpenFollowing(true)} className="hover:underline bg-transparent text-foreground hover:bg-transparent hover:text-foreground">
                <strong>{user.following.length}</strong> Following
              </Button>
              <Button className=" bg-transparent text-foreground hover:bg-transparent hover:text-foreground"><strong>{user.posts.length}</strong> Posts</Button>
            </div>

            <Button variant="outline" size="sm" onClick={() => setEditMode(true)} className="mt-3">
              Edit Profile
            </Button>
          </div>
        </CardContent>
      ...
      <Tabs defaultValue="posts" className="mt-6">
        ...
        <TabsContent value="posts" className="mt-4">
            ...
            {user.posts.length === 0 && <p className="text-muted-foreground">No posts yet</p>}
        ...
        <TabsContent value="about" className="mt-4">
          <Card>
            <CardContent className="p-4 space-y-2 text-sm">
              <p><strong>Email:</strong> {authUser?.email || "hidden"}</p>
                ...
                {PREDEFINED_AVATARS.map((url) => (
                  <Avatar
                    ...
                    className={`w-16 h-16 border-2 cursor-pointer transition ${
                      form.avatar_url === url ? "border-primary" : "border-transparent"
                    }`}
          ...
          <ScrollArea className="h-72 pr-2">
            <div className="space-y-3">
              {user.followers.map((f: any) => (
                <div key={f.id} className="flex items-center gap-3 p-2 rounded hover:bg-muted">
